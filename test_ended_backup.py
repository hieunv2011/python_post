...
import requests  # để POST dữ liệu
...
# ===== JWT cố định =====
JWT_TOKEN = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOjI2NywiZXhwIjoxNzY0NTYxODE0fQ.9BLpbjJJyPFr1yHMLJZhXnIrdZi2ncyhLxoKVGe0b2c"  # Thay bằng token thật
...
# ===== Main =====
def main():
    sid = input("Nhập session_id để test: ").strip()
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if parsed and parsed["session_id"] == sid and parsed["timestamp"] > 1763361109:
                lines.append(parsed)

    lines.sort(key=lambda x: x["timestamp"])

    if not lines:
        print("❌ Không có bản ghi thỏa điều kiện cho session_id =", sid)
        return
